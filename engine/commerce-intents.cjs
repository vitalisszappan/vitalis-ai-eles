...
const PATTERNS = [
  ...
  ['order_start', /\b(szeretnem megrendelni|meg szeretnem rendelni|rendelni szeretnek|kosarba tennem|(?:akkor |csak )?ezt kerem|csak ezt|nem kell mas|ezt akarom megrendelni|ezt szeretnem(?: megvenni)?|ezt veszem|csak (?:ezt|(?:a |az )?\w+) szeretnem|(?:az? )?(?:elsot|masodikat|harmadikat|elobbit|utobbit)(?: kerem| szeretnem| vennem meg)|inkabb (?:az? )?(?:elsot|masodikat|harmadikat|elobbit|utobbit|masikat)|(?:inkabb )?(?:a |az )?[\w -]+(?:kremet|balzsamot|szappant|sampont|csomagot) (?:kerem|szeretnem))\b/]
];
