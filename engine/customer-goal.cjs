...
function detectCustomerGoal(question) {
  ...
  if (/\b(hogyan hasznaljam|hogy hasznaljam|milyen gyakran)\b/.test(text)) return { goal: 'ask_usage', intent: 'product_usage', domain: 'product', evidence: ['followup:usage'] };
  ...
  if (/\b(van|keresek|kaphato|termek)\b/.test(text)) return { goal: 'find_product', intent: 'product_category', domain: null, evidence: ['goal:find_product'] };
