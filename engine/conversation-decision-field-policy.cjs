...
const { PROVENANCE_SOURCE_TYPES, OWNERSHIP_STATES } = require('./conversation-decision-transition-schema.cjs');
...
const FIELD_PROVENANCE_POLICY = Object.freeze(Object.fromEntries([
  ...
  ...CANONICAL_FIELDS.map((field) => [field, Object.freeze({ USER_EXPLICIT: 'ADMISSIBLE_CANDIDATE', CANONICAL_RESOLUTION: 'AUTHORITATIVE_WRITE', PROBLEM_DOMAIN_DECISION: field.includes('problemDomain') || field.includes('concernContext') ? 'AUTHORITATIVE_WRITE' : 'ADMISSIBLE_CANDIDATE', APPROVED_PRODUCT_FACT: field.includes('product') ? 'ADMISSIBLE_CANDIDATE' : 'FORBIDDEN', ASSISTANT_OUTPUT_RECOVERY: 'RECOVERY_ONLY', LEGACY_TEXT_RECOVERY: 'RECOVERY_ONLY', UNKNOWN: 'FORBIDDEN' })]),
...
module.exports = { CLASSIFICATIONS, FIELD_PROVENANCE_POLICY, FIELD_DEPENDENCY_POLICY, FIELD_REPLACEMENT_POLICY, FIELD_CONFLICT_POLICY, FIELD_INVALIDATION_POLICY, policyFor, isRecoveryOnly, isAuthorizationCapableSource, getFieldDependents, getFieldInvalidationBoundary, OWNERSHIP_STATES };