...
result = ask('Mi a különbség a Dermavital sampon és a Holt-tengeri só balzsam összetevői között?'); assert.equal(result.groundingStatus, 'unavailable'); assert.equal(result.answer, 'Erről a két termékről jelenleg nincs elég biztos információm ahhoz, hogy megbízhatóan összehasonlítsam őket.'); // 16
