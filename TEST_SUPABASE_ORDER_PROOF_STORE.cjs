...
(async()=>{const db=database(),attributionId=crypto.randomUUID(),now=new Date();
 ...
 const first=await processOrderProof(proof,{eventStore,proofStore:createSupabaseOrderProofStore({request:db.request}),verifyOrder});
 ...
 const duplicate=await processOrderProof(proof,{eventStore:createSupabaseCommerceEventStore({request:db.request}),proofStore:createSupabaseOrderProofStore({request:db.request}),verifyOrder:async()=>{throw Error('must_not_verify_duplicate')}});
